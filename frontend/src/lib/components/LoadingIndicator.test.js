...
it("can be displayed mid screen", () => {
  ...
  expect(getByLabelText(LABEL_LOADING_TEXT).getAttribute("style"))
    .toBe("position: absolute; transform: translateX(-50%); left: 50vw; top: 37vh");
});

it("can be displayed w/ a simplified indicator instead", () => {
  const { container, getByLabelText } = render(LoadingIndicator, { simple: true });
...
it("can display the simplified indicator w/ a message", () => {
  ...
  const { container, queryByLabelText, getByLabelText } =
    render(LoadingIndicator, { simple: true, message: loadingMessage });
