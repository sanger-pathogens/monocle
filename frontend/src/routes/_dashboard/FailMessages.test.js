...
it("displays the dialog w/ failures only when the button is clicked", async () => {
  ...
  expect(getByRole(roleDialog)).toBeDefined();
  expect(getByRole("heading", { name: FAIL_MESSAGES_TITLE })).toBeDefined();
  failures.forEach(({ lane, stage, issue }) => {
    expect(getByRole(roleTableCell, { name: lane })).toBeDefined();
    expect(getByRole(roleTableCell, { name: stage })).toBeDefined();
    expect(getByRole(roleTableCell, { name: issue })).toBeDefined();
  });
});
