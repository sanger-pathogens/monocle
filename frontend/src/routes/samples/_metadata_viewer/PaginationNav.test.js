...
import PaginationNav, { EVENT_NAME_PAGE_CHANGE } from "./PaginationNav.svelte";
...
it("disables First and Previous buttons only if the first page is shown", async () => {
  ...
  expect(
    getByRole(ROLE_BUTTON, { name: LABEL_FIRST_BUTTON }).disabled
  ).toBeTruthy();
  expect(
    getByRole(ROLE_BUTTON, { name: LABEL_PREV_BUTTON }).disabled
  ).toBeTruthy();
...
it("disables Next button only if the last page is shown", async () => {
  ...
  expect(
    getByRole(ROLE_BUTTON, { name: LABEL_NEXT_BUTTON }).disabled
  ).toBeFalsy();
...
it.each([
  ...
  "dispatches the page change event w/ the correct page number if %s button is clicked",
  async (labelButton, initialPageNum, expectedPageNum) => {
    const { component, getByRole } = render(PaginationNav, {
      maxNumSamplesPerPage: MAX_NUM_SAMPLES_PER_PAGE,
...
describe("sample number", () => {
  ...
  it("is only displayed if the number of samples is > 0", async () => {
    ...
    const sampleNumberIndicator =
      container.getElementsByClassName("num-samples")[0];
  ...
  it("is updated correctly on page change", async () => {
    ...
    expect(
      getByLabelText(
        `displaying samples from ${
          lastSampleNum - MAX_NUM_SAMPLES_PER_PAGE + 1
        } to ${lastSampleNum} out of ${NUM_SAMPLES}`
      )
    ).toBeDefined();
  ...
  it("shows the correct numer of samples for the first page if the total number of samples is less than the maximum number of samples per page", () => {
    ...
    expect(
      getByLabelText(
        `displaying samples from 1 to ${numSamples} out of ${numSamples}`
      )
    ).toBeDefined();
  });
  ...
  it("shows the correct numer of samples for the last page if the last page has fewer samples than the maximum number of samples per page", () => {
    ...
    expect(
      getByLabelText(
        `displaying samples from ${
          MAX_NUM_SAMPLES_PER_PAGE * (pageNum - 1) + 1
        } to ${numSamples} out of ${numSamples}`
      )
    ).toBeDefined();
  });
});
