    ...
    for this_file in _find_files(qc_data_file_name, qc_dir):
        if os.path.getsize(this_file) >= qc_data_file_min_size:
            ...
            with open(this_file, "r") as this_file_read:
                qc_data.append(
                    {"lane_id": os.path.basename(os.path.dirname(this_file)), "qc_data": json.load(this_file_read)}
                )
...
def _get_update_request_body(qc_dir):
    ...
    for this_lane in qc_data:
        ...
        # qc_data include multiple relative abundance values: sort in reverse order of timestamp, and
        # only use the first (i.e. most recent) value for each species
        species_already_found = {}
        for this_rel_abundance in reversed(sorted(qc_data.get("rel_abundance", []), key=itemgetter("timestamp"))):
            this_species = this_rel_abundance["species"]
            if this_species in rel_abun_species:
                if this_species not in species_already_found:
                    # if we reach this line, this_rel_abundance contains the most recent relative abundance value for this_species
                    try:
                        request_property = rel_abun_species[this_species]
                        this_value = float(this_rel_abundance["value"])
                        logging.debug(
                            "{} has {} rel abundance {}; adding to request as property {}".format(
                                lane_id, this_species, this_value, request_property
                            )
                        )
                        this_lane_request_data[request_property] = this_value
                        # creating a value in species_already_found will cause us to ignore any further (older)
                        # instances of this_rel_abundance that might exist for this_species
                        species_already_found[this_species] = True
                        ...
                        if 0 == len(value_as_string):
                            ...
                            logging.error(
                                'lane {} has invalid {} relative abundance data: "{}"'.format(
                                    lane_id, this_species, this_rel_abundance
                                )
                            )
                            raise e
...
def get_arguments():
    ...
    parser.add_argument(
        "-D",
        "--pipeline_qc_dir",
        help="Pipeline QC data root directory [default: {}]; can set with MONOCLE_PIPELINE_QC_DATA environment variable".format(
            default_qc_data_path
        ),
