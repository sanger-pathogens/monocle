...
class UploadMetadataHandler:
    ...
    def allowed_file_types() -> List[str]:
        ...
        return [str(key) for key in UploadMetadataHandler.file_types.keys()]
    ...
    def is_valid_file_type(file: str) -> bool:
        ...
        try:
            if file and '.' in file:
                suffix = file.split('.')[-1]
                if suffix in UploadMetadataHandler.allowed_file_types():
                    valid = True
...
class UploadInSilicoHandler:
    ...
    def allowed_file_types() -> List[str]:
        """ Return a list of allowed file type extensions """
        return [str(key) for key in UploadInSilicoHandler.file_types.keys()]
    ...
    def is_valid_file_type(file: str) -> bool:
        ...
        try:
            if file and '.' in file:
                suffix = file.split('.')[-1]
                if suffix in UploadInSilicoHandler.allowed_file_types():
                    valid = True
        except Exception as e:
            logger.warning(str(e))
