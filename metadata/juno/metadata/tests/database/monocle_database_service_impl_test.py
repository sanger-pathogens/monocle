...
class TestMonocleDatabaseServiceImpl(unittest.TestCase):
    ...
    def test_get_samples_nofilters(self) -> None:
    ...
    def test_get_samples_nofilters_noresults(self) -> None:
        self.connection.execute.return_value = []
        samples = self.under_test.get_samples({})
        self.connection.execute.assert_called_with(MonocleDatabaseServiceImpl.SELECT_ALL_SAMPLES_SQL)
        self.assertIsNotNone(samples)
    ...
    def test_filter_samples_in_one(self) -> None:
        self.connection.execute.return_value = [
            dict(sample_id='9999STDY8113123',
                lane_id='2000_2#10',
                submitting_institution_id='UniversityA',
        ...
        sample_ids = self.under_test.filter_samples_in({"serotype": ["IV"]})

        self.assertEqual(sample_ids, ['9999STDY8113123'])

    def test_filter_samples_in_multi(self) -> None:
        ...
        sample_ids = self.under_test.filter_samples_in({"serotype": ["IV"], "host_status": ["INVASIVE", "CARRIAGE"]})

        self.assertEqual(sample_ids, ['9999STDY8113124'])

    @patch('metadata.api.database.monocle_database_service_impl.MonocleDatabaseServiceImpl.filter_samples_in')
    def test_get_samples_withfilters(self, mock_filter_samples_in) -> None:
        mock_filter_samples_in.return_value = ['9999STDY8113123']
        self.connection.execute.return_value = [
            dict(sample_id='9999STDY8113123',
                lane_id='2000_2#10',
                submitting_institution_id='UniversityA',
        ...
        samples = self.under_test.get_samples({"serotype": ["IV"]})
