...
class TestConfiguration(unittest.TestCase):
    ...
    def setUp(self) -> None:
        with open(self.CONFIG_FILE) as config_file:
            ...
            app_handle = connexion.FlaskApp(
                __name__, specification_dir="../interface", options={"swagger_ui": use_swagger_ui}
            )
    ...
    def test_read_database_connection_config(self):
        with tempfile.NamedTemporaryFile("w+t") as tmp:
            ...
            tmp.seek(0)
            config = read_database_connection_config(Config(None, defaults={"monocle_database_config_file": tmp.name}))
    ...
    def test_read_spreadsheet_definition_config2(self):
        ...
        config = read_spreadsheet_definition_config(
            Config(
                ...
                defaults={
                    "spreadsheet_header_row_position": 2,
                    "spreadsheet_definition": {"a_column_name": {"title": "MyColumn"}},
                },
            )
        )
