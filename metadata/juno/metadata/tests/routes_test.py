...
class TestRoutes(unittest.TestCase):
    ...
    def test_get_filtered_samples_route_jsonified(self, mocked_jsoncall, mocked_query):
        ...
        under_test = mar.get_filtered_samples_route(  {  'metadata':    [{'name':'serotype',    'values':['IA']}],
                                                         'in silico':   [{'name':'ST',          'values':['1', '14']}],
                                                         'qc data':     [{'name':'rel_abun_sa', 'values':['99.3']}]
                                                         },
                                                      fakeDB)
        fakeDB.get_filtered_samples.assert_called_once_with(   {  'metadata':    {'serotype':['IA']},
                                                                  'in silico':   {'ST':['1', '14']},
                                                                  'qc data':     {'rel_abun_sa':['99.3']}
                                                                  }
                                                               )
        mocked_jsoncall.assert_called_once()
        self.assertEqual(under_test, ('expected', 200))
    ...
    def test_get_filtered_samples_route_no_return(self, mocked_jsoncall, mocked_query):
        ...
        under_test = mar.get_filtered_samples_route({'metadata':[{'name':'serotype', 'values':['IA']}]}, fakeDB)
        fakeDB.get_filtered_samples.assert_called_once_with( {'metadata': {'serotype':['IA']}} )
        mocked_jsoncall.assert_called_once()
        self.assertEqual(under_test, ('', 404))
    ...
    def test_get_filtered_samples_route_bad_field_name(self, mocked_jsoncall, dao_mock):
        ...
        filtered_samples, http_status = mar.get_filtered_samples_route({'metadata':[{'name':mock_field_name, 'values':mock_values}]}, dao_mock)
        dao_mock.get_filtered_samples.assert_called_once_with( { 'metadata': {mock_field_name: mock_values} } )
    ...
    def test_get_filtered_samples_route_stop_injection(self, mocked_jsoncall, mocked_query):
        ...
        for looks_iffy_to_me_guv in [  'serotype="Ia"; DO something NASTY; --',
          ...
          under_test = mar.get_filtered_samples_route({'metadata':[{'name':looks_iffy_to_me_guv, 'values':['any search term']}]}, fakeDB)
    ...
    def test_get_distinct_values_route(self, mocked_jsoncall, dao_mock):
        ...
        dao_mock.get_distinct_values.assert_called_once_with( 'metadata', mock_query['fields'], mock_query['institutions'] )
    ...
    def test_get_distinct_values_route_stop_injection(self, dao_mock):
        ...
        for looks_iffy_to_me_guv in [  'serotype="Ia"; DO something NASTY; --',
          ...
          under_test = mar.get_distinct_values_route({'fields': [looks_iffy_to_me_guv], 'institutions': ['institutionA']}, dao_mock)
    ...
    def test_get_distinct_in_silico_values_route(self, mocked_jsoncall, dao_mock):
        ...
        dao_mock.get_distinct_values.assert_called_once_with( 'in silico', mock_query['fields'], mock_query['institutions'] )
        self.assertEqual(distinct_values, mock_json)
        self.assertEqual(http_status, 200)
    ...
    def test_get_distinct_in_silico_values_route_stop_injection(self, dao_mock):
        ...
        for looks_iffy_to_me_guv in [  'serotype="Ia"; DO something NASTY; --',
          ...
          under_test = mar.get_distinct_in_silico_values_route({'fields': [looks_iffy_to_me_guv], 'institutions': ['institutionA']}, dao_mock)
          self.assertEqual(under_test, ('Invalid arguments provided', 400))
    ...
    def test_get_distinct_qc_data_values_route(self, mocked_jsoncall, dao_mock):
        ...
        dao_mock.get_distinct_values.assert_called_once_with( 'qc data', mock_query['fields'], mock_query['institutions'] )
        self.assertEqual(distinct_values, mock_json)
        self.assertEqual(http_status, 200)
    ...
    def test_get_distinct_qc_data_values_route_stop_injection(self, dao_mock):
        ...
        for looks_iffy_to_me_guv in [  'serotype="Ia"; DO something NASTY; --',
          ...
          under_test = mar.get_distinct_qc_data_values_route({'fields': [looks_iffy_to_me_guv], 'institutions': ['institutionA']}, dao_mock)
          self.assertEqual(under_test, ('Invalid arguments provided', 400))
