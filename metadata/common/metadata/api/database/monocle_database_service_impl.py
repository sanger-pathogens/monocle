...
class MonocleDatabaseServiceImpl(MonocleDatabaseService):
    ...
    INSERT_OR_UPDATE_SAMPLE_SQL = text(
        ...
        """ \
            INSERT INTO api_sample (
                sanger_sample_id, supplier_sample_name, public_name, lane_id, study_name, study_ref, selection_random, 
                country, county_state, city, submitting_institution, collection_year, collection_month, collection_day, 
                host_species, gender, age_group, age_years, age_months, age_weeks, age_days, 
                host_status, disease_type, disease_onset, isolation_source, serotype, serotype_method, infection_during_pregnancy, 
                maternal_infection_type, gestational_age_weeks, birth_weight_gram, apgar_score, ceftizoxime, ceftizoxime_method, cefoxitin, 
                cefoxitin_method, cefotaxime, cefotaxime_method, cefazolin, cefazolin_method, ampicillin, ampicillin_method, 
                penicillin, penicillin_method, erythromycin, erythromycin_method, clindamycin, clindamycin_method, tetracycline, 
                tetracycline_method, levofloxacin, levofloxacin_method, ciprofloxacin, ciprofloxacin_method, daptomycin, daptomycin_method, 
    ...
    SELECT_ALL_IN_SILICO_SQL = text(
        ...
        """ \
            SELECT
                lane_id, cps_type, ST, adhP, pheS, atr, glnA, 
                sdhA, glcK, tkt, twenty_three_S1, twenty_three_S3, AAC6APH2, AADECC, 
                ANT6, APH3III, APH3OTHER, CATPC194, CATQ, ERMA, ERMB, 
                ERMT, LNUB, LNUC, LSAC, MEFA, MPHC, MSRA, 
                MSRD, FOSA, GYRA, PARC, RPOBGBS_1, RPOBGBS_2, RPOBGBS_3, 
                RPOBGBS_4, SUL2, TETB, TETL, TETM, TETO, TETS, 
                ALP1, ALP23, ALPHA, HVGA, PI1, PI2A1, PI2A2, 
                PI2B, RIB, SRR1, SRR2, twenty_three_S1_variant, twenty_three_S3_variant, GYRA_variant, 
        ...
        with self.connector.get_transactional_connection() as con:
            for in_silico_data in in_silico_data_list:
                con.execute(
                    ...
                    twenty_three_S1_variant=self.convert_string(
                        in_silico_data.twenty_three_S1_variant
                    ),
                    twenty_three_S3_variant=self.convert_string(
                        in_silico_data.twenty_three_S3_variant
                    ),
                    GYRA_variant=self.convert_string(in_silico_data.GYRA_variant),
                    PARC_variant=self.convert_string(in_silico_data.PARC_variant),
                    RPOBGBS_1_variant=self.convert_string(
                        in_silico_data.RPOBGBS_1_variant
                    ),
                    RPOBGBS_2_variant=self.convert_string(
                        in_silico_data.RPOBGBS_2_variant
                    ),
                    RPOBGBS_3_variant=self.convert_string(
                        in_silico_data.RPOBGBS_3_variant
                    ),
                    RPOBGBS_4_variant=self.convert_string(
                        in_silico_data.RPOBGBS_4_variant
                    ),
                )
