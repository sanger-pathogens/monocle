...
class UpdateMetadataFiles:
    ...
    def generate_sql_replacements(self,data):
        ...
        ret["INSERT_OR_UPDATE_SAMPLE_SQL"] = "INSERT INTO api_sample (\n"+\
        ...
        ret["SELECT_SAMPLES_SQL"] = "SELECT\n"+\
            self.chunk_text(", ",columns,7)+\
            f"\nFROM api_sample\nWHERE\n{self.indent}sanger_sample_id IN :samples"
        ret["SELECT_ALL_SAMPLES_SQL"] = "SELECT\n"+\
            self.chunk_text(", ",columns,7)+\
            f"\nFROM api_sample\nORDER BY sanger_sample_id"

        columns = data["in_silico_data"]["spreadsheet_definition"].keys()
        ret["SELECT_ALL_IN_SILICO_SQL"] = "SELECT\n"+\
            self.chunk_text(", ",columns,7)+\
            f"\nFROM in_silico\nORDER BY lane_id"
        ret["INSERT_OR_UPDATE_IN_SILICO_SQL"] = "INSERT INTO in_silico (\n"+\
            self.chunk_text(", ",columns,7)+\
            "\n) VALUES (\n"+\
            self.chunk_text(", ",list(map(lambda c:f":{c}", columns)),7)+\
            "\n) ON DUPLICATE KEY UPDATE\n"+\
            ",\n".join(list(map(lambda c:f"{self.indent}{c} = :{c}", columns)))
        ret["SELECT_LANES_IN_SILICO_SQL"] = "SELECT\n"+\
            self.chunk_text(", ",columns,7)+\
            ...
            if entry.is_dir():
                ...
                if os.path.exists(config_path):
                    with open(config_path) as config_file:
                        ...
                        self.update_monocle_database_service_impl(data,f"{metadata_path}/{entry.name}/metadata/api/database/monocle_database_service_impl.py")
                        for (k,class_name) in [("metadata","Metadata"),("in_silico_data","InSilicoData"),("qc_data","QCData")]:
                            self.generate_dataclass_file(data[k],class_name,f"{metadata_path}/{entry.name}/metadata/api/model/{k}.py")
                        for (k,filename) in [("metadata","api_sample.sql"),("in_silico_data","in_silico.sql"),("qc_data","qc_data.sql")]:
