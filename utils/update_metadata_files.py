...
class UpdateMetadataFiles:
    ...
    def generate_code_replacements_data_handlers(self, data):
        """Generates a dict to replace variable assignments based on code in a Python file."""
        ret = {}

        columns = data["metadata"]["spreadsheet_definition"].keys()
        ret[r"^\s*def create_download_response.*Metadata.*$"] = "\n".join(
            list(map(lambda c: f'self._append_to_dict(record_dict, "{c}", entry.{c})', columns))
        )

        columns = data["in_silico_data"]["spreadsheet_definition"].keys()
        ret[r"^\s*def create_download_response.*InSilicoData.*$"] = "\n".join(
            list(map(lambda c: f'self._append_to_dict(record_dict, "{c}", entry.{c}),', columns))
        )

        columns = data["qc_data"]["spreadsheet_definition"].keys()
        ret[r"^\s*def create_download_response.*QCData.*$"] = "\n".join(
            list(map(lambda c: f'self._append_to_dict(record_dict, "{c}", entry.{c})', columns))
        )

        return ret

    def update_monocle_download_handlers(self, data, filename):
        """Updates the download_handlers.py file."""
        autogeneration_note = self.get_autogeneration_note("CODE SECTION")
        code_replacements = self.generate_code_replacements_data_handlers(data)
        # Replace existing code
        with open(filename, "r") as in_file:
            lines = in_file.readlines()
        new_code = ""
        current_method = ""
        p_begin = re.compile(r"^\s*# BEGIN AUTOMATIC INSERT\s*$")
        while len(lines) > 0:
            line = lines.pop(0)
            out = line

            for pattern in code_replacements.keys():
                p = re.compile(pattern)
                if p.match(line):
                    current_method = pattern
            if current_method in code_replacements:
                if p_begin.match(line):
                    out += f"{self.indent*3}{autogeneration_note}"
                    out += self.pad(code_replacements[current_method], 3) + "\n"
                    out += self.skip_lines_until(r"^\s*# END AUTOMATIC INSERT\s*$", lines)
            new_code += out
        with open(filename, "w") as output_file:
            _ = output_file.write(new_code)
            ...
            if entry.is_dir():
                ...
                if os.path.exists(config_path):
                    with open(config_path) as config_file:
                        ...
                        self.update_monocle_download_handlers(
                            data,
                            f"{metadata_path}/{entry.name}/metadata/api/download_handlers.py",
                        )
                        for (k, class_name) in [
                            ("metadata", "Metadata"),
