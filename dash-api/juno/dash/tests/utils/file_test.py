...
class TestFileUtil(TestCase):
    ...
    def test_format_file_size_by_calling_correct_cli_program(self):
        ...
        call_cli.assert_called_once_with(["numfmt", "--to=iec", "--suffix=B", str(bytes)], encoding="UTF-8")
    ...
    def test_zip_files(self):
        ...
        zip_files(
            PUBLIC_NAME_TO_LANE_FILES, basename=BASENAME, location=location, injected_zip_file_lib=self.ZipFileMock
        )
        ...
        num_files = reduce(lambda accum, lane_files: accum + len(lane_files), PUBLIC_NAME_TO_LANE_FILES.values(), 0)
        self.assertEqual(zipfile_instance.write.call_count, num_files)
        for public_name, files in PUBLIC_NAME_TO_LANE_FILES.items():
            for file in files:
                zipfile_instance.write.assert_any_call(file, PurePath(public_name, file.name))
