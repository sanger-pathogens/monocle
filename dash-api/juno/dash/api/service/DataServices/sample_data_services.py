...
class MonocleSampleData:
    ...
    def _apply_metadata_filters(self, filtered_samples, metadata_filters):
        logging.info(
            "{}._apply_metadata_filters filtering inital list of {} samples".format(
    ...
    def _apply_in_silico_filters(self, filtered_samples, in_silico_filters):
        logging.info(
            "{}._apply_in_silico_filters filtering inital list of {} samples".format(
                __class__.__name__, len(filtered_samples)
            )
        )
    ...
    def get_metadata_for_download(self, download_hostname, institution, category, status):
        ...
        sample_status = {"institution": institution, "category": category, "status": status}
    ...
    def get_csv_download(self, csv_response_filename, sample_status=None, sample_filters=None, download_links=None):
        ...
        *Important:*  Currently download links can only be provided where all the samples are
        ...
        if sample_status:
            ...
            csv_response_string = self.metadata_as_csv(
                sample_filters=sample_filters, download_base_url=download_base_url
            )
    ...
    def _get_samples_by_status(self, sample_status):
        ...
        try:
            ...
            logging.error(
                "{} must be passed sample_status dict including 'institution', 'category' and 'status'".format(
                    __class__.__name__
                )
            )
            raise
        ...
        logging.debug(
            "getting metadata: institution db key {} maps to dict key {}".format(institution, institution_data_key)
        )
        ...
        for this_sanger_sample_id in sequencing_status_data[institution_data_key].keys():
            if this_sanger_sample_id == API_ERROR_KEY:
                if (
                    sequencing_status_data[institution_data_key][this_sanger_sample_id]
                    == "Server Error: Records cannot be collected at this time. Please try again later."
                ):
                    logging.error(
                        "getting metadata: httpError records could not be collected for {}".format(institution)
                    )
        ...
        logging.info(
            "Found {} samples from {} with {} status {}".format(
                len(filtered_samples.keys()), institution, category, status
            )
        )
