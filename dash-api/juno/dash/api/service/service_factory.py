    ...
    def get_samples(self):
        ...
        for this_sample in all_juno_samples:
            try:
                ...
                logging.info("samples excluded because {} is not in current user's institutions list".format(
    ...
    def get_sequencing_status(self):
        ...
        for this_institution in institutions_data.keys():
            ...
            if 0 < len(sample_id_list):
                logging.debug(
                    "{}.get_sequencing_status() requesting sequencing status for samples {}".format(__class__.__name__,
                                                                                                    sample_id_list))
            ...
            if 0 < len(sample_id_list):
                ...
                for this_sample_id in sample_id_list:
                    ...
                    for this_lane in this_sequencing_status_data[this_sample_id]['lanes']:
                        if 'qc complete' == this_lane['run_status'] and this_lane['qc_complete_datetime'] and 1 == \
                                this_lane['qc_started']:
    ...
    def get_metadata(self, institution, category, status, download_base_url):
        ...
        for this_sample_id in sequencing_status_data[institution_data_key].keys():
            ...
            for this_lane in sequencing_status_data[institution_data_key][this_sample_id]['lanes']:
                ...
                if 'qc complete' == this_lane['run_status'] and this_lane['qc_complete_datetime'] and 1 == this_lane[
                    'qc_started']:
        ...
        logging.info(
            "Found {} lanes from {} with {} status {}".format(len(lane_id_list), institution, category, status))
        ...
        for this_sample_id in samples_received:
            ...
            for this_lane in sequencing_status_data[institution][this_sample_id]['lanes']:
                ...
                if 'complete_datetime' in this_lane:
                    ...
                    # get date in ISO8601 format (YYYY-MM-DD)
                    sequenced_date = datetime.strptime(this_lane['complete_datetime'], self.mlwh_datetime_fmt).strftime(
                        '%Y-%m-%d')
...
class TestDataService(DataService):
    ...
    def __init__(self, set_up: bool = True):
        DataService.__init__(self, set_up)
