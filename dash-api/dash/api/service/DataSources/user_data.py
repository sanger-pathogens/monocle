...
class UserData:
    ...
    def ldap_search_group_by_gid(self, gid):
        ...
        result_list = self.ldap_search(self.config["groups_obj"], self.config["gid_attr"], gid)
        if 0 == len(result_list):
            return None
        ...
        if len(result_list) > 1:
            logging.error(
                "The GID {} matched multiple entries in {}.{}:  it should be unique".format(
                    gid, self.config["groups_obj"], self.config["gid_attr"]
                )
            )
