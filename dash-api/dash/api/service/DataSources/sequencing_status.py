...
class MLWH_Client:
    ...
    def read_mlwh_api_config(self, mlwh_api_config_file):
        ...
        for required_param in self.required_mlwh_api_params:
            if required_param not in self.config["mlwh_api_connection"]:
                ...
                raise KeyError
        logging.info(
            "read {} to get MLWH API config: {}".format(mlwh_api_config_file, self.config["mlwh_api_connection"])
        )
    ...
    def find_by_ids(self, sample_ids):
        ...
        logging.debug(
            "{}.find_by_ids() using endpoint {}, passing list of {} sample IDs".format(
                __class__.__name__, endpoint, len(sample_ids)
            )
        )
        try:
